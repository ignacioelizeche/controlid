...
class System {
  ...
  async getAccessLogs() {
    ...
    const headers = { "Content-Type": "application/json" };
    // The device expects `fields` to be an array of field names, not an object.
    // Build the detailed fields object for local reference, but send only keys as an array.
    const fieldsObj = {
    ...
    const dataBody = {
      ...
      name: "Logs de Acesso",
      order: ["id"],
      // send only the field names as an array to satisfy device API
