...
DB_TIMEOUT = 30  # segundos para esperar si la BD está bloqueada

def _get_connection():
    """Obtiene una conexión a la BD con timeout y WAL mode configurados."""
...
def get_last_log_time(device_internal_id: int) -> Optional[int]:
    """Obtiene el tiempo del último log guardado para el dispositivo."""
    conn = _get_connection()
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT MAX(time) FROM access_logs WHERE device_internal_id = ?", (device_internal_id,))
